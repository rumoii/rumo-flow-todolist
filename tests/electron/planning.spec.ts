...
test('plans tasks, creates linked actions, searches and recovers through real IPC', async () => {
  ...
  try {
    ...
    await page.getByRole('combobox', { name: '安排方式' }).click()
    ...
    const reviewToggle = page.getByRole('button', { name: '展开完整复盘（6 问）', exact: true })
    await reviewToggle.focus()
    await reviewToggle.press('Enter')
    await expect(page.getByRole('button', { name: '收起为简版复盘（3 问）', exact: true })).toBeFocused()
    await page.getByPlaceholder('如实写下，不责备自己。').fill('需要减少切换')
    await page.getByRole('button', { name: '收起为简版复盘（3 问）', exact: true }).press('Space')
    await expect(page.getByPlaceholder('如实写下，不责备自己。')).toBeHidden()
    await expect(page.getByPlaceholder('如实写下，不责备自己。')).toHaveValue('需要减少切换')
    await expect(page.locator('.saved-pill')).toHaveText('未保存')
    ...
    await page.getByRole('searchbox', { name: '搜索关键词' }).fill('完成了计划整理')
    ...
    await page.getByRole('checkbox', { name: '选择任务 待细化的任务', exact: true }).check()
