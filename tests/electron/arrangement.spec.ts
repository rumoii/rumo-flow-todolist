...
test('arranges through real windows without overwriting drafts or resetting deadlines', async () => {
  ...
  try {
    ...
    await expect(page.getByPlaceholder('补充背景、思路或参考信息…')).toHaveValue('')
    ...
    await page.getByPlaceholder('补充背景、思路或参考信息…').fill('缓存同步后的正式修改')
    ...
    await capture.getByRole('button', { name: '保留草稿并继续' }).click()
    ...
    await page.getByPlaceholder('补充背景、思路或参考信息…').fill('未提交的详情，不能覆盖')
    ...
    await expect(page.getByPlaceholder('补充背景、思路或参考信息…')).toHaveValue('未提交的详情，不能覆盖')
