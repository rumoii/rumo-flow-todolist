...
test('keeps the core layout usable at a narrow desktop window', async ({ page }) => {
  ...
  await expect(page.locator('.workspace-search-trigger')).toBeVisible()
...
test('pins tasks, changes priority and exposes subtasks near the top of details', async ({ page }) => {
  ...
  await page.getByRole('menuitem', { name: '置顶任务' }).click()
  ...
  await page.getByRole('menuitemradio', { name: '低', exact: true }).click()
...
test('keeps saved-filter names horizontal and applies completed status before the empty state', async ({ page }) => {
  ...
  const search = page.getByPlaceholder('添加一个任务，按 Enter 保存…')
  await search.fill('?')
...
test('registers an intentional video and saves a daily flow review', async ({ page }) => {
  ...
  expect(primaryNavigation.map((label) => label.replace(/\d+/g, '').trim())).toEqual(['☷ 总计划', '✦ 收集箱', '☀ 今天', '▦ 本周', '▦ 本月', '◷ 已逾期', '♲ 回收站', '✓ 已完成'])
