...
const validDate = (value?: string | null) => value == null || validCalendarDate(value)
...
export class Repository {
  ...
  updateTask(taskId: string, input: UpdateTaskInput): Task {
    ...
    const db = getDatabase(); db.transaction(() => { db.prepare('UPDATE tasks SET title=?,list_id=?,due_date=?,due_time=?,reminder_minutes_before=?,priority=?,notes=?,sort_order=?,is_pinned=?,parent_task_id=?,updated_at=?,reminder_notified_at=NULL WHERE id=?').run(merged.title, merged.listId ?? null, merged.dueDate ?? null, merged.dueTime ?? null, merged.reminderMinutesBefore ?? null, merged.priority, merged.notes ?? '', merged.sortOrder ?? 0, merged.isPinned ? 1 : 0, merged.parentTaskId ?? null, timestamp(), taskId); if (input.sortOrder === undefined && (input.priority !== undefined && input.priority !== current.priority || input.isPinned !== undefined && input.isPinned !== current.isPinned)) db.prepare('UPDATE tasks SET sort_order=(SELECT COALESCE(MAX(sort_order),-1)+1 FROM tasks AS grouped WHERE grouped.id<>tasks.id AND grouped.deleted_at IS NULL AND grouped.parent_task_id IS NULL AND grouped.is_pinned=tasks.is_pinned AND grouped.priority=tasks.priority) WHERE id=?').run(taskId); if (input.recurrence !== undefined) { db.prepare('DELETE FROM recurrence_rules WHERE task_id=?').run(taskId); db.prepare('UPDATE tasks SET recurrence_rule_id=NULL WHERE id=?').run(taskId); if (input.recurrence) { const ruleId = newId(); db.prepare('INSERT INTO recurrence_rules(id,task_id,frequency,interval,weekdays,end_date,next_due_date,month_day) VALUES (?,?,?,?,?,?,?,?)').run(ruleId, taskId, input.recurrence.frequency, input.recurrence.interval ?? 1, JSON.stringify(input.recurrence.weekdays ?? []), input.recurrence.endDate ?? null, merged.dueDate ?? null, input.recurrence.frequency === 'monthly' && merged.dueDate ? Number(merged.dueDate.slice(8, 10)) : null); db.prepare('UPDATE tasks SET recurrence_rule_id=? WHERE id=?').run(ruleId, taskId) } } if (input.tagIds !== undefined) this.saveTags(taskId, input.tagIds) })()
    return this.getTask(taskId)
  }
  organizeTask(taskId: string, input: { isPinned: boolean; priority: TaskPriority; orderedIds: string[] }): Task {
    if (!Array.isArray(input.orderedIds) || input.orderedIds.some((id) => typeof id !== 'string') || !input.orderedIds.includes(taskId)) throw new Error('任务顺序无效')
    if (typeof input.isPinned !== 'boolean' || !['none', 'low', 'medium', 'high'].includes(input.priority)) throw new Error('任务分组无效')
    const db = getDatabase()
    db.transaction(() => {
      if (db.prepare('UPDATE tasks SET is_pinned=?,priority=?,updated_at=? WHERE id=? AND deleted_at IS NULL AND parent_task_id IS NULL').run(input.isPinned ? 1 : 0, input.priority, timestamp(), taskId).changes !== 1) throw new Error('任务不存在')
      const groupIds = (db.prepare('SELECT id FROM tasks WHERE deleted_at IS NULL AND parent_task_id IS NULL AND is_pinned=? AND priority=?').all(input.isPinned ? 1 : 0, input.priority) as Array<{ id: string }>).map((item) => item.id)
      if (new Set(input.orderedIds).size !== input.orderedIds.length || groupIds.length !== input.orderedIds.length || groupIds.some((id) => !input.orderedIds.includes(id))) throw new Error('任务顺序不完整')
  ...
  organizeList(id: string, input: { isPinned: boolean; orderedIds: string[] }): TaskList {
    if (!Array.isArray(input.orderedIds) || input.orderedIds.some((item) => typeof item !== 'string') || !input.orderedIds.includes(id)) throw new Error('清单顺序无效')
    if (typeof input.isPinned !== 'boolean') throw new Error('清单分组无效')
    const db = getDatabase()
    db.transaction(() => {
      if (db.prepare('UPDATE task_lists SET is_pinned=?,updated_at=? WHERE id=?').run(input.isPinned ? 1 : 0, timestamp(), id).changes !== 1) throw new Error('清单不存在')
      const groupIds = (db.prepare('SELECT id FROM task_lists WHERE is_pinned=?').all(input.isPinned ? 1 : 0) as Array<{ id: string }>).map((item) => item.id)
      if (new Set(input.orderedIds).size !== input.orderedIds.length || groupIds.length !== input.orderedIds.length || groupIds.some((item) => !input.orderedIds.includes(item))) throw new Error('清单顺序不完整')
  ...
  private validateBackup(payload: BackupPayload): void {
    ...
    this.validateSettings(payload.settings as Partial<AppSettings>)
