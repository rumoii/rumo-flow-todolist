...
export class TaskRepository {
  ...
  validateTask(input: Partial<CreateTaskInput>): void { if (!validDate(input.dueDate) || !validDate(input.recurrence?.endDate))
  ...
  listTasks(query: TaskQuery = {}): Task[] {
    ...
    const params: Record<string, unknown> = { today: new Date().toISOString().slice(0, 10), next7: new Date(Date.now() + 7 * 86400000).toISOString().slice(0, 10) }
    ...
    if (query.due === 'overdue')
      clauses.push("tasks.due_date<:today AND tasks.status='active'")
    if (query.due === 'next7')
  ...
  updateTask(taskId: string, input: UpdateTaskInput): Task {
    ...
    db.transaction(() => { db.prepare('UPDATE tasks SET title=?,list_id=?,due_date=?,due_time=?,reminder_minutes_before=?,priority=?,notes=?,sort_order=?,is_pinned=?,parent_task_id=?,updated_at=?,reminder_notified_at=NULL WHERE id=?').run(merged.title, merged.listId ?? null, merged.dueDate ?? null, merged.dueTime ?? null, merged.reminderMinutesBefore ?? null, merged.priority, merged.notes ?? '', merged.sortOrder ?? 0, merged.isPinned ? 1 : 0, merged.parentTaskId ?? null, timestamp(), taskId); if (input.sortOrder === undefined && (input.priority !== undefined && input.priority !== current.priority || input.isPinned !== undefined && input.isPinned !== current.isPinned))
      ...
      this.saveTags(taskId, input.tagIds); })()
    return this.getTask(taskId)
  }
