...
app.whenReady().then(() => {
  ...
  reminderScheduler = new ReminderScheduler(repository, (taskId) => desktop?.showMain(taskId))
  registerIpcHandlers({ onTasksChanged: () => reminderScheduler?.reschedule(), openQuickCapture: () => desktop?.showCapture(), desktopStatus: () => ({ globalShortcut: desktop?.shortcut ?? repository.getSettings().globalShortcut, globalShortcutRegistered: desktop?.shortcutRegistered ?? false }), onSettingsChanged: () => desktop?.registerShortcut(repository.getSettings().globalShortcut) })
