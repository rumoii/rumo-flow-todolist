...
const filteredTasks = computed(() => {
  ...
  return tasks.value.filter(task => {
    ...
    if (!activeFilter.value && activeView.value !== 'completed' && task.status === 'completed') return false
    if (activeListId.value && task.listId !== activeListId.value) return false
    if (activeView.value === 'inbox' && task.listId !== null) return false
    if (activeView.value === 'today' && !isTodayTask(task)) return false
    if (activeView.value === 'upcoming' && (!task.dueDate || task.dueDate < todayIso.value || task.dueDate > isoDate(new Date(currentDate.value.getTime() + 30 * 86400000)))) return false
    if (activeView.value === 'week' && (!task.dueDate || task.dueDate < todayIso.value || task.dueDate > isoDate(weekEnd.value))) return false
    if (temporaryTagId.value && !task.tags.some(tag => tag.id === temporaryTagId.value)) return false
    if (activeFilter.value && !matchesTaskFilter(task, activeFilter.value.criteria, currentDate.value)) return false
...
const emptyState = computed(() => {
  if (search.value) return { title: '没有匹配的任务', hint: '换一个关键词试试', canCreate: false }
  if (activeFilter.value) return { title: `“${activeFilter.value.name}”暂无匹配任务`, hint: '可以调整筛选条件，或继续添加符合条件的任务。', canCreate: false }
  if (activeView.value === 'completed') return { title: '还没有完成的任务', hint: '完成的任务会集中显示在这里。', canCreate: false }
  if (activeView.value === 'upcoming') return { title: '近期没有安排', hint: '未来 30 天内的任务会显示在这里。', canCreate: false }
  if (activeView.value === 'week') return { title: '本周还没有安排', hint: '可以从其他视图拖动任务到本周。', canCreate: false }
...
function dateLabel(value: string | null) {
  if (!value) return '无日期'
  if (value === todayIso.value) return '今天'
  const tomorrow = new Date(currentDate.value); tomorrow.setDate(currentDate.value.getDate() + 1)
...
async function createTask(title = quickTitle.value) {
  const parsed = parseQuickAdd(title, lists.value, tags.value, currentDate.value); const clean = parsed.input.title.trim(); if (!clean) return
  try {
...
async function dropTaskBefore(target: Task) {
  ...
  try {
    ...
    if (dragged.isPinned === target.isPinned) await persistTaskOrder(ordered)
    else await organizeTask(dragged, target.isPinned, target.priority, ordered)
...
async function dropListBefore(target: TaskList) {
  ...
  try {
    ...
    ordered.splice(targetIndex < 0 ? ordered.length : targetIndex, 0, dragged)
    if (dragged.isPinned === target.isPinned) await persistListOrder(ordered)
    else await organizeList(dragged, target.isPinned, ordered)
...
async function onWeekDrop(event: DragEvent, targetDate: string) { event.preventDefault(); const task = tasks.value.find(item => item.id === draggedTaskId.value); endTaskDrag(); if (!task || task.dueDate === targetDate) return; try { const updated = hasApi() ? await window.todoApi.tasks.update(task.id, { dueDate: targetDate }) : { ...task, dueDate: targetDate }; Object.assign(task, updated); notify(`已移动到${dateLabel(targetDate)}`) } catch { notify('日期更新失败') } }
...
function handleShortcut(event: KeyboardEvent) {
  ...
  if (event.key === 'Escape') {
    ...
    else if (pendingListDelete.value) pendingListDelete.value = null
...
watch([pendingDelete, pendingListDelete, settingsOpen, shortcutsOpen], async (values, previous) => {
  const isOpen = values.some(Boolean)
  const wasOpen = previous.some(Boolean)
  if (isOpen && !wasOpen) lastFocusedElement = document.activeElement as HTMLElement | null
  await nextTick()
  if (isOpen) {
    const dialogs = document.querySelectorAll<HTMLElement>('.dialog-backdrop [role="dialog"]')
    const dialog = dialogs[dialogs.length - 1]
    if (dialog && !dialog.contains(document.activeElement)) (dialogFocusable(dialog)[0] ?? dialog).focus()
  } else if (wasOpen) {
...
<template>
  <div class="app-shell" @click="closeMenus">
    <aside class="sidebar">
      ...
      <button class="primary-action" @click="focusQuickAdd"><span aria-hidden="true">＋</span> 新建任务 <kbd>Ctrl N</kbd></button>
      <nav class="nav-group">
        <button :class="['nav-item', { active: activeView === 'inbox' }]" @click="activeView = 'inbox'"><span class="nav-icon" aria-hidden="true">✦</span> 收集箱 <em>{{ tasks.filter(task => task.status === 'active' && task.listId === null).length }}</em></button>
        <button :class="['nav-item', { active: activeView === 'today' }]" @click="activeView = 'today'"><span class="nav-icon" aria-hidden="true">☀</span> 今天 <em>{{ tasks.filter(isTodayTask).length }}</em></button>
        <button :class="['nav-item', { active: activeView === 'week' }]" @click="activeView = 'week'"><span class="nav-icon" aria-hidden="true">▦</span> 本周</button>
        <button :class="['nav-item', { active: activeView === 'upcoming' }]" @click="activeView = 'upcoming'"><span class="nav-icon" aria-hidden="true">◷</span> 即将到期</button>
        <button :class="['nav-item', { active: activeView === 'completed' }]" @click="activeView = 'completed'"><span class="nav-icon" aria-hidden="true">✓</span> 已完成 <em>{{ completedCount }}</em></button>
      </nav>
      <nav class="flow-nav-section" aria-label="心流"><button :class="['nav-item', { active: activeView === 'flow' }]" @click="activeView = 'flow'"><span class="nav-icon" aria-hidden="true">◌</span> 心流 <em>记录与复盘</em></button></nav>
      <div class="sidebar-section saved-filter-section">
        ...
        <Transition name="filter-composer">
          <div v-if="filterComposerOpen" class="filter-composer-motion">
            <div class="list-composer filter-composer-form" @click.stop>
              ...
              <div class="filter-field-row"><span>状态</span><SelectField v-model="newFilterStatus" aria-label="筛选状态" :options="[{ value: 'active', label: '进行中' }, { value: 'completed', label: '已完成' }, { value: 'all', label: '全部' }]" /></div>
              <div class="filter-field-row"><span>清单</span><SelectField v-model="newFilterListId" aria-label="筛选清单" :options="[{ value: 'any', label: '任意清单' }, { value: 'inbox', label: '收集箱' }, ...sortedLists.map(list => ({ value: list.id, label: list.name }))]" /></div>
              <div class="filter-field-row"><span>优先级</span><SelectField v-model="newFilterPriority" aria-label="筛选优先级" :options="[{ value: 'any', label: '任意优先级' }, { value: 'high', label: 'P1' }, { value: 'medium', label: 'P2' }, { value: 'low', label: 'P3' }, { value: 'none', label: '无' }]" /></div>
              <div class="filter-field-row"><span>标签</span><SelectField v-model="newFilterTagId" aria-label="筛选标签" :options="[{ value: 'any', label: '任意标签' }, ...tags.map(tag => ({ value: tag.id, label: `#${tag.name}` }))]" /></div>
              <div class="filter-field-row"><span>日期</span><SelectField v-model="newFilterDue" aria-label="筛选日期" :options="[{ value: 'any', label: '任意日期' }, { value: 'today', label: '今天' }, { value: 'overdue', label: '已逾期' }, { value: 'next7', label: '未来 7 天' }, { value: 'none', label: '无日期' }]" /></div>
        ...
        <div v-for="filter in savedFilters" :key="filter.id" class="saved-filter-row">
          <button :class="['nav-item', { active: activeView === `filter:${filter.id}` }]" @click="activeView = `filter:${filter.id}`"><span class="nav-icon" aria-hidden="true">⌕</span><span class="list-name">{{ filter.name }}</span></button>
    ...
    <Transition name="dialog"><div v-if="pendingDelete" class="dialog-backdrop" @click.self="pendingDelete = null" @keydown="trapDialogFocus"><section class="confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="delete-title" tabindex="-1"><div class="dialog-icon">!</div><h2 id="delete-title">删除这个任务？</h2><p>“{{ pendingDelete.title }}”及其子任务将被永久删除，此操作无法撤销。</p><div class="dialog-actions"><button class="cancel-button" @click="pendingDelete = null">取消</button><button class="delete-button" @click="removeTask(pendingDelete); pendingDelete = null">确认删除</button></div></section></div></Transition>
    <Transition name="dialog"><div v-if="pendingListDelete" class="dialog-backdrop" @click.self="pendingListDelete = null" @keydown="trapDialogFocus"><section class="confirm-dialog list-delete-dialog" role="dialog" aria-modal="true" aria-labelledby="delete-list-title" tabindex="-1"><div class="dialog-icon">!</div><h2 id="delete-list-title">删除清单“{{ pendingListDelete.name }}”？</h2><p>请选择如何处理清单中的任务。</p><label class="delete-policy"><input v-model="listDeletePolicy" type="radio" value="keep" /><span><strong>保留任务</strong><small>任务将转为“无清单”</small></span></label><label class="delete-policy danger-policy"><input v-model="listDeletePolicy" type="radio" value="delete" /><span><strong>同时删除任务</strong><small>清单中的任务和子任务将永久删除</small></span></label><div class="dialog-actions"><button class="cancel-button" @click="pendingListDelete = null">取消</button><button class="delete-button" @click="confirmListDelete">确认删除</button></div></section></div></Transition>
    <Transition name="dialog"><div v-if="settingsOpen" class="dialog-backdrop" @click.self="settingsOpen = false" @keydown="trapDialogFocus"><section class="settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title" tabindex="-1"><header><div><p>Rumo-Flow</p><h2 id="settings-title">设置与数据</h2></div><button class="icon-button" aria-label="关闭设置" @click="settingsOpen = false">×</button></header><div class="settings-section"><div><strong>界面主题</strong><p>选择适合当前环境的明暗外观。</p></div><SelectField v-model="settings.theme" aria-label="界面主题" :options="[{ value: 'light', label: '浅色' }, { value: 'dark', label: '深色' }]" @change="saveSettings" /></div><div class="settings-section"><div><strong>内容密度</strong><p>舒适模式留白更多，紧凑模式显示更多任务。</p></div><SelectField v-model="settings.density" aria-label="内容密度" :options="[{ value: 'comfortable', label: '舒适' }, { value: 'compact', label: '紧凑' }]" @change="saveSettings" /></div><div class="settings-section"><div><strong>每日视频额度</strong><p>心流默认允许登记 3 条，达到后温和提醒。</p></div><input v-model.number="settings.dailyVideoLimit" class="settings-number" aria-label="每日视频额度" type="number" min="0" max="10" @change="saveSettings" /></div><div class="settings-section"><div><strong>每日复盘提醒</strong><p>应用或托盘运行时，在设定时间提醒一次。</p></div><div class="review-reminder-setting"><label><input v-model="settings.reviewReminderEnabled" type="checkbox" @change="saveSettings" /> 启用</label><input v-model="settings.reviewReminderTime" aria-label="复盘提醒时间" type="time" :disabled="!settings.reviewReminderEnabled" @change="saveSettings" /></div></div><div class="settings-section"><div><strong>全局快捷键</strong><p>{{ desktopStatus.globalShortcutRegistered ? `已启用 ${desktopStatus.globalShortcut}` : `${desktopStatus.globalShortcut} 注册失败，请检查快捷键冲突` }}</p></div><button class="secondary-button" @click="shortcutsOpen = true">快捷键帮助</button></div><div class="settings-section tag-management"><div class="tag-management-heading"><strong>标签管理</strong><p>创建、改名和调整颜色；删除只解除任务关联。</p></div><div class="tag-create-row"><input v-model="settingsTagName" placeholder="新标签名称" @keydown.enter="createTagFromSettings" /><button class="secondary-button" @click="createTagFromSettings">创建</button></div><div v-if="tags.length" class="managed-tag-list"><div v-for="tag in tags" :key="tag.id" class="managed-tag-row"><input v-model="managedTagDrafts[tag.id].name" class="managed-tag-name" :aria-label="`标签名称 ${tag.name}`" /><input v-model="managedTagDrafts[tag.id].color" type="color" :aria-label="`标签颜色 ${tag.name}`" /><button class="secondary-button" @click="updateManagedTag(tag)">保存</button><button class="danger-link" @click="deleteManagedTag(tag)">删除</button></div></div><p v-else class="tag-empty">还没有标签</p></div><div class="settings-section"><div><strong>导出数据备份</strong><p>将任务、标签、筛选、心流复盘和视频思考保存为 JSON。</p></div><button class="secondary-button" @click="exportBackup">导出备份</button></div><div class="settings-section"><div><strong>从备份恢复</strong><p>恢复前自动保存当前数据；v1、v2、v3 备份均可导入。</p></div><button class="secondary-button" @click="importBackup">选择文件</button></div><footer>数据仅保存在当前设备 · Rumo-Flow</footer></section></div></Transition>
    <Transition name="dialog"><div v-if="shortcutsOpen" class="dialog-backdrop" @click.self="shortcutsOpen = false" @keydown="trapDialogFocus"><section class="confirm-dialog shortcut-dialog" role="dialog" aria-modal="true" aria-labelledby="shortcut-title" tabindex="-1"><h2 id="shortcut-title">键盘快捷键</h2><div class="shortcut-row"><span>聚焦新任务</span><kbd>Ctrl N</kbd></div><div class="shortcut-row"><span>全局快速捕获</span><kbd>Ctrl Alt Space</kbd></div><div class="shortcut-row"><span>关闭浮层</span><kbd>Esc</kbd></div><div class="shortcut-row"><span>打开帮助</span><kbd>?</kbd></div><button class="save-button" @click="shortcutsOpen = false">知道了</button></section></div></Transition>
