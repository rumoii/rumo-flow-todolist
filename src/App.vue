...
const viewHint = computed(() => activeView.value === 'today' ? `${today.toLocaleDateString('zh-CN', { month: 'long', day: 'numeric', weekday: 'long' })} · 把注意力放在最重要的事上` : activeView.value === 'flow' ? '让输入慢下来，把思考留下来' : activeView.value === 'upcoming' ? '未来 30 天的安排' : activeView.value === 'week' ? '接下来七天的轻量排程' : activeView.value === 'completed' ? '已经完成的任务' : '这个清单中的任务')
...
function isTodayTask(task: Task) {
  return !task.parentTaskId && task.status === 'active' && (!task.dueDate || task.dueDate <= todayIso)
}
...
const completedTodayCount = computed(() => tasks.value.filter(task => task.status === 'completed' && task.completedAt?.slice(0, 10) === todayIso).length)
  ...
  <div class="app-shell" @click="closeMenus">
    <aside class="sidebar">
      ...
      <div class="sidebar-section saved-filter-section">
        ...
        <div v-for="filter in savedFilters" :key="filter.id" :class="['nav-item', { active: activeView === `filter:${filter.id}` }]" role="button" tabindex="0" @click="activeView = `filter:${filter.id}`"><span>⌕</span><span class="list-name">{{ filter.name }}</span><button class="inline-remove" aria-label="删除筛选" @click.stop="removeFilter(filter)">×</button></div>
      </div>
