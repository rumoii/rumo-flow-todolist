...
<template>
  <div class="app-shell" @click="closeMenus">
    ...
    <main class="main-content">
      <header class="page-header"><Transition name="title" mode="out-in"><div :key="activeView"><p class="eyebrow">{{ viewHint }}</p><h1>{{ viewTitle }}</h1></div></Transition><div class="header-actions"><label class="search-box"><span>⌕</span><input v-model="search" placeholder="搜索任务" /></label><button class="avatar">R</button></div></header>
    ...
    <Transition name="dialog"><div v-if="settingsOpen" class="dialog-backdrop" @click.self="settingsOpen = false"><section class="settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title"><header><div><p>Rumo-Flow</p><h2 id="settings-title">设置与数据</h2></div><button class="icon-button" aria-label="关闭设置" @click="settingsOpen = false">×</button></header><div class="settings-section"><div><strong>导出数据备份</strong><p>将清单、任务、子任务和重复规则保存为 JSON 文件。</p></div><button class="secondary-button" @click="exportBackup">导出备份</button></div><div class="settings-section"><div><strong>从备份恢复</strong><p>恢复前会自动保存当前数据；导入失败不会修改现有任务。</p></div><button class="secondary-button" @click="importBackup">选择文件</button></div><footer>数据仅保存在当前设备 · Rumo-Flow 0.2.0</footer></section></div></Transition>
