...
const videoDraft = ref({ title: '', sourceUrl: '', author: '' })
...
const inputOptions = computed(() => [{ value: 'none', label: '暂不记录输入' }, { value: 'other', label: '书、文章或谈话等其他输入' }, ...day.value.videos.map((video) => ({ value: `video:${video.id}`, label: `视频 · ${video.title}` }))])
...
function hydrateDrafts() {
  ...
  reviewDraft.value = { didWell: review.didWell, didNotWell: review.didNotWell, reflection: review.reflection, inputText: review.inputText, outputText: review.outputText, tomorrowExpectation: review.tomorrowExpectation }
  reviewInputChoice.value = review.inputType === 'video' && review.inputVideoId ? `video:${review.inputVideoId}` : review.inputType
  videoDrafts.value = Object.fromEntries(day.value.videos.map((video) => [video.id, { title: video.title, sourceUrl: video.sourceUrl, author: video.author, thought: video.thought }]))
}
...
async function selectDay(date: string) { if (!date || date > todayIso.value) return; selectedDate.value = date; await loadDay() }
...
async function addVideo() {
  ...
  if (!videoDraft.value.title.trim() || !videoDraft.value.sourceUrl.trim()) { setNotice('请填写视频标题和来源链接'); return }
  ...
  if (warnings.length && !window.confirm(`${warnings.join('，')}。仍要登记下一条吗？`)) return
  try {
    const created = hasApi() ? await window.todoApi.flow.createVideo({ date: selectedDate.value, ...videoDraft.value }) : null
    videoDraft.value = { title: '', sourceUrl: '', author: '' }
    await refresh()
    if (created) {
      try { await window.todoApi.desktop.openExternal(created.sourceUrl); setNotice('已登记，观看后记得补上自己的思考') }
      catch { setNotice('视频已登记，但来源链接未能打开') }
    }
  } catch (error) { setNotice(error instanceof Error ? error.message : '视频登记失败') }
}
...
async function saveVideo(video: VideoReflection) {
  ...
  try { if (hasApi()) await window.todoApi.flow.updateVideo(video.id, draft); await refresh(); setNotice(draft.thought.trim() ? '视频思考已保存' : '视频信息已保存，思考仍待补充') }
  catch (error) { setNotice(error instanceof Error ? error.message : '视频记录保存失败') }
}
async function removeVideo(video: VideoReflection) {
  if (!window.confirm(`删除“${video.title}”的记录吗？`)) return
  try { if (hasApi()) await window.todoApi.flow.removeVideo(video.id); await refresh(); setNotice('视频记录已删除') }
    ...
    <div class="flow-layout">
      <div class="flow-primary">
        <section class="flow-card">
          <header class="section-heading"><div><small>今日输入</small><h2>先登记，再观看</h2></div><span :class="['quota-pill', { warning: overLimit }]">{{ day.videos.length }}/{{ day.review.videoLimit }}</span></header>
          <div v-if="isToday" class="video-composer">
            <label><span>视频标题</span><input v-model="videoDraft.title" placeholder="这条视频讲了什么？" /></label>
            <label><span>来源链接</span><input v-model="videoDraft.sourceUrl" type="url" placeholder="https://…" @keydown.enter="addVideo" /></label>
            <label><span>作者（选填）</span><input v-model="videoDraft.author" placeholder="账号或创作者" /></label>
            <button class="primary-button" @click="addVideo">登记并打开视频</button>
          </div>
          <p v-else class="history-hint">历史日期可以补写和修改思考，但只能在今天登记新视频。</p>
          <div v-if="loading" class="flow-empty">正在加载…</div>
          <div v-else-if="!day.videos.length" class="flow-empty">今天还没有登记视频。没有刷，也是一种清醒的选择。</div>
          <div v-else class="video-list">
            <article v-for="video in day.videos" :key="video.id" class="video-entry">
              <div class="video-entry-heading"><div><span :class="['thought-state', { done: videoDrafts[video.id]?.thought.trim() }]">{{ videoDrafts[video.id]?.thought.trim() ? '已思考' : '待补思考' }}</span><strong>{{ video.title }}</strong><small>{{ video.sourcePlatform }}<template v-if="video.author"> · {{ video.author }}</template></small></div><button class="text-button" @click="openVideo(video.sourceUrl)">打开来源 ↗</button></div>
              <div v-if="videoDrafts[video.id]" class="video-edit-grid">
                <label><span>标题</span><input v-model="videoDrafts[video.id].title" /></label>
                <label><span>来源</span><input v-model="videoDrafts[video.id].sourceUrl" type="url" /></label>
                <label><span>作者</span><input v-model="videoDrafts[video.id].author" placeholder="选填" /></label>
        ...
        <section class="flow-card review-card">
          <header class="section-heading"><div><small>今日复盘</small><h2>{{ selectedDateLabel }}</h2></div><span :class="['saved-pill', { saved: day.review.savedAt }]">{{ day.review.savedAt ? '已保存' : '未保存' }}</span></header>
