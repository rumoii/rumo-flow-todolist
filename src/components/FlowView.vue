...
let historyScroll = 0
const dayWord = computed(() => isToday.value ? '今天' : '当天')
...
async function editHistory(date: string) {
  ...
  if (!await selectDay(date)) return
...
async function backfill() {
  ...
  if (!await selectDay(backfillDate.value)) return
  await setActiveTab('input')
...
watch(() => navigation?.flowTarget.value, async target => {
  ...
  await setActiveTab(target.videoId ? 'input' : 'review')
  await nextTick()
...
<template>
  ...
  <div ref="flowRoot" class="flow-view" :inert="loading || addingVideo || drafts.paused.value || drafts.saving.value">
    ...
    <div class="flow-top-actions">
      ...
      <button class="text-button" :aria-expanded="calendarOpen" aria-controls="flow-calendar" @click="calendarOpen = !calendarOpen">日历跳转</button>
        ...
        <Transition name="flow-tab" mode="out-in">
          <section v-if="activeTab === 'input'" id="flow-panel-input" key="input" class="flow-card" role="tabpanel" aria-labelledby="flow-tab-input">
            ...
            <div class="video-composer">
              <label><span>{{ isToday ? '粘贴准备观看的视频链接' : '补录当天看过的视频链接' }}</span><input v-model="videoUrl" type="url" placeholder="https://…" @keydown.enter="addVideo" /></label>
  ...
  <FlowFacts :date="selectedDate" />
      ...
      <aside v-show="calendarOpen" id="flow-calendar" class="flow-history">
        <section class="flow-card calendar-card">
          ...
          <div class="calendar-grid">
            <button v-for="cell in calendarCells" :key="cell.key" :disabled="!cell.date || cell.date > todayIso" :class="['calendar-day', { selected: cell.date === selectedDate, today: cell.date === todayIso, saved: daySummary(cell.date)?.reviewSaved, over: daySummary(cell.date)?.overLimit }]" @click="calendarDay(cell.date)">
...
<style scoped>
:global(:root[data-theme="dark"] .flow-tabs button.active) { background: var(--accent-soft); color: #c7bcff; }
