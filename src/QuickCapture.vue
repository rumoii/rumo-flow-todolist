...
async function submit() { const parsed = parseQuickAdd(title.value, lists.value, tags.value); if (!parsed.input.title.trim()) return; await window.todoApi.tasks.create(parsed.input); title.value = ''; message.value = '已加入收集箱'; setTimeout(() => { message.value = '' }, 1200) }
</script>
