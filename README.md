...
当前源码将「标签管理」独立为侧栏业务页面；视频额度在「心流 → 输入记录」调整，提醒时间在「心流 → 每日复盘」调整。「设置」仅保留外观、快捷键、数据与备份，并使用随主题变化的 Windows 标题栏。这些调整尚未包含在已发布的 RC 安装包中。
...
数据库迁移 9 在升级已有数据库前，将一致性 SQLite 快照保存到数据库目录下的 `backups/pre-planning-*.sqlite`。旧任务保持编号、截止日期和草稿，执行计划初始为未安排；任务草稿升级为 v2。回退需关闭所有应用进程并恢复升级前快照，不能用旧应用打开升级后的数据库或导入 v5。工作区修改不会自动更新既有 Release。
